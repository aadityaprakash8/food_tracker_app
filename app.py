...
def index():
    ...
    if request.method == 'POST':
        ...
        dt = datetime.strptime(date, '%Y-%m-%d') # YYYY-MM-DD
        database_date = datetime.strftime(dt, '%Y%m%d' ) #YYYYMMDD
...
@app.route('/view/<date>', methods = ['GET', 'POST']) # date in format YYYYMMDD here 20200920
def view(date):
    db = get_db()
